...
Follow the following steps to set up a new deployment target server for manual deployments using the `cap <env> deploy` command, where `<env>` is the value given in the Capistrano Environment column, above, as well as automated Travis deployments.
